...
class processTransactions extends MobilePayments{
	...
	*/
	...
	public function account_balance($accBalJson){
		...
		file_put_contents('AccountBalanceFeedBack.txt', print_r($data, true));
	...
	*/
	...
	public function b2b_process($b2bJson){
		...
		*/
		...
		// echo $data['TransactionID'] = $b2bJson->Result->TransactionID;
		// echo $DebitAccountCurrentBalance;
		// echo $InitiatorAccountCurrentBalance;
		/** Processing Values */
		//1. Amount
		$Amount =  $data['Amount'] = $b2bJson->Result->ResultParameters->ResultParameter['3'];
		foreach($Amount as $key => $value)
		{
			$Amount = $value;
		}
		//2.Transaction ID;
		$TransactionID = $data['TransactionID'] = $b2bJson->Result->TransactionID;
		//3.TransCompletedTime
		$TransCompletedTime =  $data['TransCompletedTime'] = $b2bJson->Result->ResultParameters->ResultParameter['4'];
		foreach($TransCompletedTime as $key => $value)
		{
			$TransCompletedTime = $value;
		}
		//4.DebitPartyCharges
		$DebitPartyCharges =  $data['DebitPartyCharges'] = $b2bJson->Result->ResultParameters->ResultParameter['5'];
		foreach($DebitPartyCharges as $key => $value)
		{
			$DebitPartyCharges = $value;
		}
		//5.CreditPartyPublicName
		$CreditPartyPublicName =  $data['CreditPartyPublicName'] = $b2bJson->Result->ResultParameters->ResultParameter['6'];
		foreach($CreditPartyPublicName as $key => $value)
		{
			$ReceiverPartyPublicName = $value;
		}
		//6.DebitPartyPublicName
		$DebitPartyPublicName =  $data['DebitPartyPublicName'] = $b2bJson->Result->ResultParameters->ResultParameter['7'];
		foreach($DebitPartyPublicName as $key => $value)
		{
			$DebitPartyPublicName = $value;
		}
		//7.CreditAccountBalance
		$CreditAccountBalance =  $data['CreditAccountBalance'] = $b2bJson->Result->ResultParameters->ResultParameter['8'];
		foreach($CreditAccountBalance as $key => $value)
		{
			$CreditAccountBalance = $value;
		}
		//8.Currency
		$Currency =  $data['Currency'] = $b2bJson->Result->ResultParameters->ResultParameter['9'];
		foreach($Currency as $key => $value)
		{
			$Currency = $value;
		}
		//9.DebitAccountBalance
		$DebitAccountBalance =  $data['DebitAccountBalance'] = $b2bJson->Result->ResultParameters->ResultParameter['0'];
		foreach($DebitAccountBalance as $key => $value)
		{
			$DebitAccountBalance = $value;
		}
		//9.InitiatorAccountCurrentBalance
		$InitiatorAccountCurrentBalance =  $data['InitiatorAccountCurrentBalance'] = $b2bJson->Result->ResultParameters->ResultParameter['1'];
		foreach($InitiatorAccountCurrentBalance as $key => $value)
		{
			$InitiatorAccountCurrentBalance = $value;
		}
		//9.DebitAccountCurrentBalance
		$DebitAccountCurrentBalance =  $data['DebitAccountCurrentBalance'] = $b2bJson->Result->ResultParameters->ResultParameter['2'];
		foreach($DebitAccountCurrentBalance as $key => $value)
		{
			$DebitAccountCurrentBalance = $value;
		}
		
		
		$this->insert_b2b_response($TransactionID, $InitiatorAccountCurrentBalance,$DebitAccountCurrentBalance, $Amount, $DebitAccountBalance, $TransCompletedTime, $DebitPartyCharges, $ReceiverPartyPublicName, $Currency);
		return true;
	}
	...
	*
	*/
	public function b2bs_process($b2bJson){
		$accountBalanceResult = $b2bJson->Result->ResultParameters->ResultParameter;
		$accountBalanceResult = json_encode($accountBalanceResult);
		$accountBalanceResult = json_decode($accountBalanceResult, TRUE);
		...
		// var_dump($accountBalanceResult);
		// foreach ( $accountBalanceResult as $balance ) {
